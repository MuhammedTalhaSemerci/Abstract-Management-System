...
<?php
    ...
    if(isset($button_infos->{"documents"}))
    {
        for($i=0; $i<count($button_infos->{"documents"});$i++)
        {
                ...
                echo ' <a style="margin-left:20px;" href="view/abstracts/sponsor_documents/Bitki Koruma Kongresi/'.$user_index["sponsor_company"]."/".strval($button_infos->{"documents"}[$i]).'" download> '.strval($button_infos->{"documents"}[$i]).'</a> ';
