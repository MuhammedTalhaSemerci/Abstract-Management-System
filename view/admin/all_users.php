...
<body>
    ...
    <div style="display:flex;">
        <select name="search" id="" style="width:200px;margin-left:20px;margin-top:20px;" class="form-control">
            ...
            <option value="abstract@0" >Bildiri Yüklemeyenler</option>
            <option value="abstract@1" >Bildiri Yükleyenler</option>
        </select>
        
        <button style="margin-left:20px;margin-top:20px;" class="form-control" onclick="congress_payment_date_control()">Geç/Erken kayıtları kontrol et</button>
    </div>
...
echo '<div><table style="margin-left:20px;">
        <tr>
            ...
            <td ><font color="red"><b>Üye ID: </b></font></td>
            <td ><font color="red"><b>Üye Düzenle: </b></font></td>
            <td ><font color="red"><b>Ödeme Durumu: </b></font></td>
            <td ><font color="red"><b>Ek İşlemler</b></font></td>
            ...
            <td ><font color="red"><b>Ödenen ücret: </b></font></td>
...
<tr>
    ...
    <td style="padding:4px;">'.$all_users[$i]["id"].'</td>

    <td> <a href="/admin_index?sayfa=profile_change&admin_user_id='.$all_users[$i]["id"].'">Düzenle</a> </td>';
    ...
    if(intval($all_users[$i]["uye_yetki"]) == 3)
    {
        echo '<font color="blue">Bu üyenin kongre kaydını değiştiremezsiniz.</font>';
    }
    else
    {
        ...
        echo '<select id="withdrawal_state_'.$i.'" name="withdrawal_state_'.$i.'" onChange="admin_withdrawal_update(\'withdrawal_state_'.$i.'\')">';
    ...
    echo '<td><button class="form-control" onclick="admin_manuel_price_mailer(\''.$all_users[$i]["uye_mail"].'\')" style="height:auto;" title="Mail gönder"><i class="fas fa-paper-plane"></i></button></td>
    ...
    {
        $user_withdrawal_types = explode("/",$user_withdrawal[0][1]);
        ...
        {
            echo '<td>Kongre Katılımcısı  ('.$user_withdrawal_types[0].'/'.$user_withdrawal_types[2].')</td>';
        }
    ...
    {
        if(isset($user_withdrawal))
        {
            $user_withdrawal_types = explode("/",$user_withdrawal[0][1]);
            echo '<td><b>Bilim kurulu üyesi  ('.$user_withdrawal_types[0].'/'.$user_withdrawal_types[2].')</b></td>';
        }
        else
        {
    ...
    echo '
    <td><input type="number" class="member_paid_val" id="'.$all_users[$i]["id"].'" value="'.$all_users[$i]["uye_odenen"].'"></td>
</tr>
...
congress_payment_date_control = function()
{
    var y ;
     if (confirm("İşlemi gerçekleştirmek istediğinizden emin misiniz ?") == true) {
        setTimeout(function(){window.location.href='congress_payment_date_control';},10);
     } else {
         y = "işleminiz iptal edildi!";
         alert(y); 
     }
}
...
$( "select[name=search]" )
  .change(function () {
    ...
    let str ="";
        ...
        postForm('/admin_search?sayfa=all_users', {value:str,users:1});
  ...
  function admin_manuel_price_mailer(user_mail)
  {
    ...
    var y ;
     if (confirm("Mail gönderme işlemini onaylıyor musunuz?") == true) 
     {
        postForm('/admin_manuel_mailer?mail_type=price', {user_mail:user_mail,contant:summernote});
     } 
     else 
     {
         y = "Mail gönderme işlemi sonlandırıldı!";
...
function admin_withdrawal_update(id) {
    ...
    var y ;
    if (confirm("İşlemi onaylıyor musunuz?") == true) 
    {      
        postForm('/admin_withdrawal_update?'+str, {value:str});
    } 
    else 
    {
        y = "İşlem sonlandırıldı!";
        alert(y); 
    }

}

$(".member_paid_val").change(function(){

    var paid_val = this.value;
    ...
    if (confirm("İşlemi onaylıyor musunuz?") == true) 
     {      
         if(paid_val > 0)
         {
            $.post('/admin_paid_val_update?value='+paid_val+'&id='+member_id, {},function(){
                this.value = paid_val;
            });
         }
         else
         {
            $.post('/admin_paid_val_update?value=0&id='+member_id, {},function(){
                this.value = paid_val;
            });
         }
