...
<body>
    ...
    <a href="/admin_index?sayfa=all_abstracts">Geri Dön</a>
    <form action="/admin_abstract_authorization?abstract_id=<?php echo $update_abstract["abstract_no"];?>" method="post"> 
...
<div class="row">
    ...
    <div class="col-md-11 col-lg-11">
        <table class="table" style="margin-left:20px; margin-right:20px;">
            <tr>
                <td ><font color="red"><b>Atama: </b></font></td>
    ...
    {
        if($update_abstract["abstract_no"] == $admin_all_abstracts[$i][4])
        {
            $abstract_author_infos .= $admin_all_abstracts[$i][11];
        }
    }
    ...
    for($i=0;$i<count($referee_users);$i++)
    {
        ...
        for($a=0;$a<count($admin_all_abstracts);$a++)
        {
            if($admin_all_abstracts[$a][10] != null)
            {
                $referee_ids = json_decode($admin_all_abstracts[$a][10]);
        ...
        for($z=0;$z<count($admin_all_abstracts);$z++)
        {
            ...
            if($admin_all_abstracts[$z][8] !=null && $admin_all_abstracts[$z][9] != null)
            {
                $edit_requests = json_decode($admin_all_abstracts[$z][8]);
                $referee_comments = json_decode($admin_all_abstracts[$z][9]);
