...
<meta name="theme-color" content="#ffffff">
   ...
   for($i=0;$i<count($abstract_websites);$i++ ){
    ...
    $sponsors = json_decode($abstract_websites[0]["sponsors"]);
   ?>
                ...
                <label for="withdrawal_state">Ücret ödeme durumu: </label>
                    <?php
                        ...
                        echo '<select name="withdrawal_state" id="">';
                            ...
                            echo '<option value="id='.$user_index["id"].'&withdrawal=0"'.(intval($withdrawal) == 0)?"selected":"".'>Ödeme yapılmadı</option>';
                            echo '<option value="id='.$user_index["id"].'&withdrawal=1"'.(intval($withdrawal) == 1)?"selected":"".'>Ödeme yapıldı</option>'; 
                            echo '<option value="id='.$user_index["id"].'&withdrawal=2"'.(intval($withdrawal) == 2)?"selected":"".'>Ödeme geçersiz</option>'; 
                    ...
                    echo '<label for="withdrawal_state_change">Kongre kaydını düzenle: </label>';

                      
                        
                    echo '<select name="withdrawal_state_change" id="">';
                        //düzenlenecek alan
                        echo '<option>Seçim Yapınız</option>';
                       
                        echo '<option value="Kayit_ucreti=Öğrenci Kaydı/200/Erken Kayıt&Kayit_onay=0&id='.$user_index["id"].'" >Öğrenci Kaydı(Onaysız-Erken)</option>';
                        echo '<option value="Kayit_ucreti=Normal Kayıt/300/Erken Kayıt&Kayit_onay=0&id='.$user_index["id"].'" >Normal Kayıt(Onaysız-Erken)</option>'; 
                        echo '<option value="Kayit_ucreti=Katılımcı Kaydı/250/Erken Kayıt&Kayit_onay=0&id='.$user_index["id"].'" >Katılımcı Kaydı(Onaysız-Erken)</option>'; 
                        echo '<option value="Kayit_ucreti=Öğrenci Kaydı/200/Erken Kayıt&Kayit_onay=1&id='.$user_index["id"].'" >Öğrenci Kaydı(Onaylı-Erken)</option>';
                        echo '<option value="Kayit_ucreti=Normal Kayıt/300/Erken Kayıt&Kayit_onay=1&id='.$user_index["id"].'" >Normal Kayıt(Onaylı-Erken)</option>'; 
                        echo '<option value="Kayit_ucreti=Katılımcı Kaydı/250/Erken Kayıt&Kayit_onay=1&id='.$user_index["id"].'" >Katılımcı Kaydı(Onaylı-Erken)</option>'; 

                        echo '<option value="Kayit_ucreti=Öğrenci Kaydı/200/Geç Kayıt&Kayit_onay=0&id='.$user_index["id"].'" >Öğrenci Kaydı(Onaysız-Geç)</option>';
                        echo '<option value="Kayit_ucreti=Normal Kayıt/300/Geç Kayıt&Kayit_onay=0&id='.$user_index["id"].'" >Normal Kayıt(Onaysız-Geç)</option>'; 
                        echo '<option value="Kayit_ucreti=Katılımcı Kaydı/250/Geç Kayıt&Kayit_onay=0&id='.$user_index["id"].'" >Katılımcı Kaydı(Onaysız-Geç)</option>'; 
                        echo '<option value="Kayit_ucreti=Öğrenci Kaydı/200/Geç Kayıt&Kayit_onay=1&id='.$user_index["id"].'" >Öğrenci Kaydı(Onaylı-Geç)</option>';
                        echo '<option value="Kayit_ucreti=Normal Kayıt/300/Geç Kayıt&Kayit_onay=1&id='.$user_index["id"].'" >Normal Kayıt(Onaylı-Geç)</option>'; 
                        echo '<option value="Kayit_ucreti=Katılımcı Kaydı/250/Geç Kayıt&Kayit_onay=1&id='.$user_index["id"].'" >Katılımcı Kaydı(Onaylı-Geç)</option>';
                        
                        echo '<option value="Kayit_ucreti=Burslu Kayıt/--/Erken Kayıt&Kayit_onay=1&id='.$user_index["id"].'" >Burslu Kayıt</option>';
                        echo '<option value="kayit_durum=sil&id='.$user_index["id"].'" >Kaydı Sil</option>';
                        
                    echo '</select>';
            
                    echo '<label for="sponsor_info">Sponsor Yetkisi Ata: </label>';
                    echo '<select name="sponsor_info" id="">';
                        //düzenlenecek alan
                        echo '<option value="id='.$user_index["id"].'">Seçim Yapınız</option>';
                        for($i=0;$i<count($sponsors);$i++)
                        {
                            $selected_or_not = ($user_index["sponsor_company"] != null && $user_index["sponsor_company"] == $sponsors[$i])? "selected":"";
                            echo '<option value="sponsor_company='.$sponsors[$i].'&id='.$user_index["id"].'"'.$selected_or_not.'>'.$sponsors[$i].'</option>';
                        }
                    ...
                    echo '<label for="sponsor_info">Stand Görevlisi Ata: </label>';
                    echo '<select name="stand_info" id="">';
                    //düzenlenecek alan
                    echo '<option value="id='.$user_index["id"].'">Seçim Yapınız</option>';
                    for($i=0;$i<count($sponsors);$i++)
                    {
                        $selected_or_not = ($user_index["stand_info"] != null && $user_index["stand_info"] == $sponsors[$i])? "selected":"";
                        echo '<option value="sponsor_company='.$sponsors[$i].'&id='.$user_index["id"].'"'.$selected_or_not.'>'.$sponsors[$i].'</option>';
                    }
...
<br><br>
	...
	<form action="/admin_user_update?id=<?php echo $user_index["id"];?>" method="POST">
        ...
        <textarea name="burslulukaciklama" style="width:100%;" ><?php echo $user_index["uye_aciklama"] ;?></textarea><br>
        <br>
        <br>
...
$( "select[name=authority]" )
  ...
  $( "select[name=sponsor_info]" ).change(function () {
    let str ="";
    str = $( "select[name=sponsor_info] option:selected" ).val();
    
    postForm('/admin_sponsor_authorization_update?'+str, {value:str});

  });



  $( "select[name=stand_info]" ).change(function () {
    let str ="";
    str = $( "select[name=stand_info] option:selected" ).val();
    
    postForm('/admin_stand_authorization_update?'+str, {value:str});
...
var value2 =0;
  ...
  $( "select[name=withdrawal_state_change]" )
  .change(function () {
    ...
    let str ="";
        $( "select[name=withdrawal_state_change] option:selected" ).each(function() {
        str += $( this ).val();
        });
        
        postForm('/registration_replace?'+str, {value:str});
